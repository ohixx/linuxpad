...
class GlobalHotkeyListener:
    ...
    def parse(self, h_str):
        parts = h_str.lower().replace(" ", "").split("+")
        keys = set()
        for p in parts:
            if p == "ctrl": keys.add(keyboard.Key.ctrl_l)
            elif p == "alt": keys.add(keyboard.Key.alt_l)
            elif p == "shift": keys.add(keyboard.Key.shift_l)
            elif p.startswith("f") and p[1:].isdigit(): keys.add(getattr(keyboard.Key, p, None))
    ...
    def register(self, h_str, f_path):
        try:
            k = self.parse(h_str)
            if k: self.hotkeys[k] = f_path
        except: pass

    def clear(self): self.hotkeys.clear()

    def on_press(self, key):
        self.current_keys.add(key)
        frozen = frozenset(self.current_keys)
        for k, fp in self.hotkeys.items():
            if k.issubset(frozen): self.callback(fp)

    def on_release(self, key): self.current_keys.discard(key)
...
class AudioDeviceManager:
    ...
    def _format_display_name(name):
        ...
        prefixes = ['alsa_output.', 'alsa_input.', 'bluez_sink.', 'bluez_source.']
        ...
        suffixes = ['.monitor', 'analog stereo', 'pro audio']
...
class DeviceSelectDialog(QDialog):
    ...
    def load_devices(self):
        ...
        for dev in devices:
            ...
            if dev['id'] == self.current_target or dev.get('node_id') == self.current_target:
                item.setBackground(QBrush(QColor("#2d4a2d")))

            if any(x in dev['name'].lower() for x in ['virtual', 'null', 'mic', 'soundboard']):
                font = item.font()
                font.setBold(True)
                item.setFont(font)
        ...
        if not devices:
            item = QListWidgetItem("No devices found. Try 'pactl list sinks short'")
...
class SoundpadWindow(QMainWindow):
    ...
    def play_selected(self):
        ...
        if row >= 0 and row < len(self.sounds):
    ...
    def stop_sound(self):
        if self.proc_mic:
            try:
                self.proc_mic.send_signal(signal.SIGINT)
                self.proc_mic.wait(timeout=0.1)
            except:
                try: self.proc_mic.kill()
                except: pass
            self.proc_mic = None

        if self.proc_local:
            try:
                self.proc_local.send_signal(signal.SIGINT)
                self.proc_local.wait(timeout=0.1)
            except:
                try: self.proc_local.kill()
                except: pass
            self.proc_local = None
    ...
    def set_hotkey(self, row):
        ...
        hk, ok = QInputDialog.getText(self, "Set Hotkey", "Key combo (e.g. F1, Ctrl+1):", text=curr)
        if ok:
    ...
    def save_config(self):
        ...
        with open(CONFIG_FILE, "w") as f:
            json.dump({
                "sounds": self.sounds,
                "target": self.target,
                "vol_mic": self.vol_mic,
                "vol_local": self.vol_local
            }, f, indent=2)
